...
def add_submodule(repo):
    ...
    directory_of_repo_info = os.path.join(data_set_path, repo)
    ...
    if os.path.exists(path_of_repo_info):
        ...
        os.makedirs(data_set_infos_owners, exist_ok=True)
        if not os.path.exists(os.path.join(data_set_infos_owners, repo_name)):
            process1 = subprocess.Popen(args=f"cp -r {directory_of_repo_info} {data_set_infos_owners}",shell=True)
            process1.wait()
        ...
        with open(path_of_repo_info) as json_file:
            ...
            if not os.path.exists(git_directory):
                ...
                process2 = subprocess.Popen(args=f"git submodule add --name {repo_owner}-{repo_name} {repo_url}.git {repo_name}", cwd=repo_father_address, shell=True)
                process2.wait()
...
if __name__ == '__main__':
    ...
    with open(os.path.join(data_set_infos_directory, 'modules.csv'), 'w', newline='') as output_file:
